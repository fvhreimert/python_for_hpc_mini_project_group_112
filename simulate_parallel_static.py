# simulate_parallel_static_minimal.py
...
def load_data(load_dir, bid):
    SIZE = 512
    u = np.zeros((SIZE + 2, SIZE + 2), dtype=np.float64) # Use float
    try:
        u[1:-1, 1:-1] = np.load(join(load_dir, f"{bid}_domain.npy"))
        interior_mask = np.load(join(load_dir, f"{bid}_interior.npy")).astype(bool) # Ensure bool
        # Basic shape check (minimal)
        if u[1:-1, 1:-1].shape != (SIZE, SIZE) or interior_mask.shape != (SIZE, SIZE):
             # Silently return None on shape mismatch or let later code fail
             return None, None
    except Exception:
        # Silently return None on any loading error
        return None, None
...
def summary_stats(u, interior_mask):
    ...
    if u_interior.size == 0: return {} # Return empty dict if no interior
...
def worker_process_floorplan(args_tuple):
    """Runs Jacobi and stats for one floorplan. Expects tuple input."""
    u0, interior_mask, bid, max_iter, abs_tol = args_tuple # Unpack
...
if __name__ == '__main__':
    ...
    # --- Get Arguments ---
    if len(sys.argv) < 3:
        # Print usage to stderr to keep stdout clean for CSV data
    ...
    # --- Load Data (Serial Pre-load) ---
    with open(join(LOAD_DIR, 'building_ids.txt'), 'r') as f:
        all_building_ids = f.read().splitlines()

    if N > len(all_building_ids): N = len(all_building_ids) # Adjust N if needed
    building_ids_subset = all_building_ids[:N]

    # Pre-load data into memory (like original script)
    loaded_data = [] # Store tuples of (u0, mask, bid) for valid loads
    for bid in building_ids_subset:
        u0, interior_mask = load_data(LOAD_DIR, bid)
        if u0 is not None: # Only store if loaded successfully
            loaded_data.append((u0, interior_mask, bid))
        else:
            # Silently skip floorplans that fail to load
            pass

    if not loaded_data:
        print("Error: No floorplan data loaded successfully.", file=sys.stderr)
        sys.exit(1)
    ...
    # --- Parallel Jacobi Execution (Timed) ---
    start_time = time.time()

    results = []
    # Use multiprocessing Pool with map for static scheduling
    try:
        with multiprocessing.Pool(processes=P) as pool:
            # pool.map distributes task_args across P workers
            results = pool.map(worker_process_floorplan, task_args)
    except Exception as e:
         print(f"Error during parallel processing: {e}", file=sys.stderr)
         # results will be empty or incomplete

    end_time = time.time()
    duration = end_time - start_time
    print(f"Processing {num_loaded} floorplans with {P} workers took {duration:.3f} seconds.", file=sys.stderr) # Time to stderr

    # --- Print Results (CSV to stdout) ---
    stat_keys = ['mean_temp', 'std_temp', 'pct_above_18', 'pct_below_15']
    print('building_id,' + ','.join(stat_keys)) # CSV header

    for bid, stats in results:
        # Ensure stats dict is not empty and format output
        if stats:
            ...
            # Print NaN for floorplans with empty stats (e.g., no interior or load error)
